...
test('should update a user\'s relationships', async t => {
  try {
    ...
    t.is(user.relationship('friends').data[2].id, '3');
    t.is(user.relationship('friends').data[0]._related, false);
    t.is(user.retrieve('friends').length, 3);
    t.truthy(user3 instanceof Node);
    t.is(user3.attribute('name'), 'Joe');
    t.is(user3.relationship('friends').data[1].id, '1');
    t.is(user3.relationship('friends').data[1]._related, true);
  } catch (err) {
    t.fail(err.message);
  }
});
